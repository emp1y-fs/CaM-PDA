...
The [reproduction guide](REPRODUCIBILITY.md) links the source downloads, exact frame manifests, both configurations and numerical loss/label definitions. The inference examples are runnable. Full training replay still requires packaging the original per-source preprocessing, frozen caches and research trainer; the reference methods are not an executable training pipeline.

Initial adaptation uses seed 2026090412, four accumulated one-frame microbatches, AdamW and gradient clipping at 1. Learning rates are 5e-6 for the backbone, 1e-5 for the decoder/condition projection, 1e-4 for routers, 2e-4 for residual experts and 1e-3 for gates. Weight decay is 1e-4 except for gates. The learning-rate multiplier follows a cosine from 1 to 0.2 over 2,016 updates. Routing is soft for the first 20 updates, then hard with straight-through gradients. Validation uses independent sparse gates. Each epoch shuffles the fixed 672 frames with seed 2026090412 + epoch; the effective batch is four. The objective is per-frame AbsRel plus 0.01 independent router BCE. [Configuration](../reproduction/configs/initial_stage.json).
