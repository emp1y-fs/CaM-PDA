...
The portable Linux CUDA pipeline reproduced the original real frame-32 result exactly: 921,600 depth pixels, all three conditions and 46,970 accepted anchors. The real browser-upload path also reproduced that result exactly. Linux CPU inference was exercised on a 90×160 aligned view using FP32. Windows native CUDA inference and the seven portable contract tests also ran successfully.
...
See `benchmarks/release_verification.json` for measured checks. The unit suite tests units, calibration, output isolation, deterministic sampling, CPU KNN edge cases, raw-anchor restoration and browser request contracts without downloading weights.
